...
- Manga OCR text now stays hidden until you tap a speech bubble: a tap reveals that bubble — on a near-opaque white plate so it stays legible even over solid-black artwork — and looks the tapped word up, while tapping empty artwork hides the revealed bubbles again. A revealed bubble shows a small copy button that copies its whole text, and page turns play a right-to-left slide animation (disabled on e-ink).

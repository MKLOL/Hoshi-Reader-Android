...
- Manga tutor defaults use localized resources and request a complete word table with furigana/romaji before grammar notes; saved prompts retain their existing text (`AiChatSettingsRepositoryTest`). Existing setups keep their saved prompt: `writeAll` persists `promptText` on any ChatGPT settings change, so only never-saved setups get the new default.

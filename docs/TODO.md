...
- Manga statistics are wired to the shared `statistics.json` sidecar while presenting manga-specific page units in the reader overflow Statistics sheet; statistics are always on for EPUB and manga (no enable/autostart settings) and the Statistics screens (`features/statistics`, reached from the Books bar and Settings) aggregate every book's `statistics.json` (`ReadingStatisticsOverviewTest`, `ReadingStatisticsConsistencyTest` guard sheet/screen agreement); manga OCR characters read per day live in `manga_statistics.json` (`MangaTextReadCounterTest`), fed only by forward turns while tracking; both sidecars sync over HTTP as `books/{syncId}/statistics` and `.../manga_statistics` with a per-day merge (`HttpSyncStatisticsSync`, debounced reader pushes via `HttpSyncStatisticsPushScheduler`, `SyncIntegrationTest.statisticsMergePerDayAcrossDevices…`); adjacent manga pages are also preloaded through a small generated-HTML cache and bounded image-file warmup. Both readers also write a local usage log (`features/usage`: one NDJSON file per day in `filesDir/usage-log`; `ReaderUsageSession` ties reading spans to the tracker's start/stop and logs page turns, lookups, bubble reveals/translations/copies and screenshot requests; `UsageLogTest`, `ReaderUsageSessionTest`, `UsageDaySummaryTest`). It is not synced or backed up yet. The Statistics screen leads with a Today card from it (`StatisticsTodayCard`) and a Trends tab draws 14/30/90-day charts with a trailing 3-day average for reading time, characters, lookups, lookups per bubble, translate presses and translate presses per bubble (`StatisticsTrendCharts`, `StatisticsTrendsTest`, `UsageStatisticsTest`); per-bubble rates divide window sums, and log-based charts start at the first logged day.

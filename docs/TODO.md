...
- Manga statistics are wired to the shared `statistics.json` sidecar while presenting manga-specific page units in the reader overflow Statistics sheet; statistics are always on for EPUB and manga (no enable/autostart settings) and Settings -> Statistics aggregates every book's `statistics.json` (`ReadingStatisticsOverviewTest`); manga OCR characters read per day live in the Android-only `manga_statistics.json` (already sync-excluded; `MangaTextReadCounterTest`), fed only by forward page turns; adjacent manga pages are also preloaded through a small generated-HTML cache and bounded image-file warmup.
